  ...
  return (
    <div className="font-sans text-slate-800">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-800 uppercase tracking-tight">Laporan & Kasir</h1>
            <p className="text-slate-500 text-sm mt-1">Pantau pesanan aktif dan rekap pendapatan harian.</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div>
                  ...
                  <p className="text-2xl font-black text-slate-800">Rp {totalRevenue.toLocaleString('id-ID')}</p>
              </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div>
                  <p className="text-xs text-green-500 font-bold uppercase mb-1 tracking-wider">Tunai (Cash)</p>
                  <p className="text-2xl font-black text-green-600">Rp {totalCash.toLocaleString('id-ID')}</p>
              </div>
              <div className="bg-green-50 p-3 rounded-xl text-green-600"><Wallet size={28}/></div>
          </div>

          {/* Total QRIS */}
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              <div>
                  <p className="text-xs text-purple-500 font-bold uppercase mb-1 tracking-wider">Non-Tunai (QRIS/Online)</p>
                  <p className="text-2xl font-black text-purple-600">Rp {totalQRIS.toLocaleString('id-ID')}</p>
              </div>
              <div className="bg-purple-50 p-3 rounded-xl text-purple-600"><CreditCard size={28}/></div>
          </div>
      </div>
      ...
      {/* TABEL PESANAN */}
      <div className="bg-white rounded-b-xl shadow-sm border border-slate-200 border-t-0 overflow-hidden min-h-[400px]">
              ...
              displayOrders.map((order) => (
                  ...
                  <td className="px-6 py-4">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                          <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${order.type_order === 'Dine In' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                              {order.type_order}
                          </span>
                  ...
                  <td className="px-6 py-4">
                      <div className="flex flex-col items-start gap-1.5">
                          ...
                          {order.status === 'Cancelled' ? (
                              ...
                              <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded border border-green-100 flex items-center gap-1"><CheckCircle size={12}/> Selesai</span>
                          ) : order.status === 'Paid' ? (
                              ...
                              <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded border border-orange-100 animate-pulse">Menunggu Proses</span>
                          )}
                  ...
                  <td className="px-6 py-4 text-right">
                      ...
                      {activeTab === 'active' && (
                          <>
                              ...
                              className="px-4 py-2 bg-green-600 text-white hover:bg-green-700 rounded-lg text-xs font-bold flex items-center gap-1 shadow-md hover:shadow-green-200 transition transform hover:-translate-y-0.5"
                              >
      ...
      {selectedOrder && (
          ...
          <div className="bg-white rounded-xl w-full max-w-sm shadow-2xl overflow-hidden scale-100 animate-in zoom-in-95">
