...
export default function EventsPage() {
  ...
  if (loading) return <div className="p-10 text-center text-slate-500 font-bold">Memuat Data Event...</div>
          ...
          {events.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                          <th className="p-5 text-xs font-bold text-slate-500 uppercase tracking-wider">Poster</th>
                          <th className="p-5 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Event</th>
                          <th className="p-5 text-xs font-bold text-slate-500 uppercase tracking-wider">Waktu Pelaksanaan</th>
                      ...
                      {events.map((item) => (
                          <tr key={item.id} className="hover:bg-slate-50 transition">
                              <td className="p-5 w-24">
                                  <div className="h-24 w-16 bg-slate-200 rounded-lg overflow-hidden relative border border-slate-200 shadow-sm">
                                      {item.image ? (
                              ...
                              <td className="p-5">
                                  <div className="flex items-center gap-2 text-slate-600 font-medium text-sm bg-slate-50 px-3 py-1.5 rounded-lg w-fit">
                                      <Clock size={16} className="text-blue-500"/>
                                      {item.description || 'Waktu belum diatur'}
                                  </div>
                                  <div className="flex items-center gap-2 text-slate-500 font-medium text-xs mt-1 ml-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    {/* Input Nama Event */}
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Nama Event / Acara</label>
                    ...
                    {/* Input Waktu */}
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Waktu Pelaksanaan</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Lokasi Event</label>
                        <div className="relative">
                    ...
                    {/* Input Poster Upload */}
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Upload Poster Event</label>
